...
def main():
    ...
    print()
    print("Etapa 3 - productor -> Logstash  (ts_indexado - ts_ingesta)")
    documentos = _traer(INDICE_TRADES, ["ts_ingesta", "ts_indexado", "origen"])
    # Los documentos anteriores al sello no traen el campo. Se descartan aqui en
    # vez de filtrar en la consulta para que el conteo diga cuantos se usaron.
    con_sello = [d for d in documentos if d.get("ts_indexado")]
    if not con_sello:
        print("  Sin documentos con `ts_indexado`.")
        print()
        print("  Lo pone Logstash con un filtro `ruby` al principio del pipeline. Si el")
        print("  campo no existe, o Logstash corre con una configuracion anterior, o los")
        print("  documentos consultados se indexaron antes de anadirlo.")
    else:
        _percentiles("Kafka -> Logstash", _a_ms("ts_ingesta", "ts_indexado", con_sello))
        print()
        print("  Cubre: publicacion en Kafka, espera en el topic, consumo de Logstash y")
        print("  paso por sus filtros. NO incluye el `bulk` hacia Elasticsearch ni el")
        print("  refresco del indice, que ocurren despues del sello.")
        print()
        print("  Se mide con UN SOLO reloj -el del contenedor de Logstash frente al del")
        print("  productor, ambos del mismo host-, asi que a diferencia de la etapa 1 no")
        print("  arrastra desfase entre maquinas y la mediana es fiable.")
        print()
        print("  Por que NO se usa @timestamp para esto: Logstash lo deriva de ts_evento,")
        print("  de modo que la resta daria cero siempre. Era el defecto de la primera")
        print("  version de esta prueba, que reportaba 0,00 ms en los tres percentiles.")

    print()
    print("Suma de las tres etapas")
    print("  Etapa 1 + Etapa 3 dan la latencia de un trade desde el exchange hasta que")
    print("  es consultable. La Etapa 2 es de las metricas, que van por otro camino y")
    print("  no se suman a la de los trades.")
    print()
    print("  Queda fuera de la medicion el `refresh_interval` de Elasticsearch (1 s por")
    print("  defecto): un documento indexado no aparece en las busquedas hasta el")
    print("  siguiente refresco. Es un limite conocido y no se disimula.")
